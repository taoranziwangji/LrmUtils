...
public class FileUtil {
    ...
    public static boolean saveBitmapToFile(String path, Bitmap bm, String fileName, boolean isfileNameAFullName) throws IOException {
        return saveBitmapToFile(path, bm, fileName, isfileNameAFullName, 80);
    }
     ...
     * @throws IOException
     */
    public static boolean saveBitmapToFile(String path, Bitmap bm, String fileName, boolean isfileNameAFullName, int quality) throws IOException {
    ...
    /**
     * 获取文件最后修改时间，时间格式yyyy-MM-dd HH:mm:ss
     *
     * @param path
     * @return
     */
    public static String getFileLastEditTime(String path) {
        File f = new File(path);
        if (!FileUtil.isFileExists(f)) {
            return TimeNowUtil.getStringDateTime();
        }
        long time = f.lastModified();
        String strTime = TimeChangeUtil.millisecondToDate(time);
        return strTime;
    }
