...
/**滑动时，可扩展的listview的父标签可以停留的ExpandableListView*/
public class vdoPinnedHeaderExpandableListView extends ExpandableListView implements OnScrollListener {
    private static final String TAG = "lrm";
    ...
    public vdoPinnedHeaderExpandableListView(Context context) {
        super(context);
        initView();
    }

    public vdoPinnedHeaderExpandableListView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initView();
    }

    public vdoPinnedHeaderExpandableListView(Context context, AttributeSet attrs, int defStyle) {
    ...
    protected void refreshHeader() {
        ...
        int group = getPackedPositionGroup(getExpandableListPosition(pos));
            ...
            MyLog.d(TAG, "refreshHeader firstVisibleGroupPos=" + firstVisibleGroupPos);


        if (group == firstVisibleGroupPos + 1) {
            View view = getChildAt(1);
            if (view == null) {
                MyLog.w(TAG, "Warning : refreshHeader getChildAt(1)=null");
                return;
            }
